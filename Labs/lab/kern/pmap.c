...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	uint32_t sz = (uint32_t) (((long long) 1)  << 32) - KERNBASE;
	boot_map_region(kern_pgdir,
					KERNBASE,
					ROUNDUP(sz, PGSIZE),
					0,
...
page_init(void)
{
	...
	// free pages!
	/**
	 * 1. Understand you are dealing with physical or virtual address.
	 * 2. Constants(like KERNBASE) in JOS can be different from xv6.
	*/
	/* Physical page 0 */
	pages[0].pp_ref = 1;
	...
	/** 
	 * Extended memory
	 * The exact amount of memory used by kernel code & data is not pre-defined.
	 * It is determined by linker, as `end` used in boot_alloc(). And more
	 * physical memory has been allocated by boot_alloc() for page directory
	 * and the list of PageInfo. Note that boot_alloc() allocates physical 
	 * memory linearly and returns virtual address.
	 */
...
//
/** 
 * Similar to walkpgdir() in xv6. 
 * You should be careful about the return value of walkpgdir(), as there are
 * 2 things you should check.
 * 1. If the return value is NULL, then the page table does not exist.
 * 2. Even if the return value is not NULL, you still have to check. The
 * return value is a pointer to the PTE, however, the PTE could be present or
 * not.
 * A typical snippet would be:
 * pte_t *ret = pgdir_walk(pgdir, va, ifCreate);
 * if (ret) {
 * 		if (*ret & PTE_P) {
 * 			...
 * 		} 
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	uint32_t pgdir_idx, pgtbl_idx, offset;
	...
	} else {	// The page table is not present
		struct PageInfo *pginfo_ptr;
		if (!create || (pginfo_ptr = page_alloc(1)) == NULL) {
			return NULL;
		}
		// Allocate page table, increment ref count.
		pginfo_ptr->pp_ref = 1;

		// Reset page directory entry
		memset(pgdir + pgdir_idx, 0, sizeof(pde_t));
		
		// Update page directory entry:
		// 1. Address of page table;
		// 2. Set bits.
		physaddr_t pgtable_phyaddr = page2pa(pginfo_ptr);
		pgdir[pgdir_idx] = pgtable_phyaddr | PTE_P | PTE_W | PTE_U;	

		pgtable = (pte_t *) KADDR(pgtable_phyaddr);
	}
	...
	if (pte_p) {
		if (*pte_p & PTE_P) {
			if (PTE_ADDR(*pte_p) == page2pa(pp)) {	
				// mapping the same physcial page to the same virtual address
				// Update permission
				*pte_p = page2pa(pp) | perm | PTE_P;
				return 0;
			} else {
				// remove previous mapped page
