...
readseg(uint32_t pa, uint32_t count, uint32_t offset)
{
	...
	while (pa < end_pa) {
		...
		// YY: at this point, physical memory is the memory we are operating 
		// on. Virtual memory is not enabled yet.
...
readsect(void *dst, uint32_t offset)
{
	...
	// YY: `in` and `out` are instructions for transfering data between I/O
	// ports. `b` specifies that 8-bit word is read/written. Detailes are not
